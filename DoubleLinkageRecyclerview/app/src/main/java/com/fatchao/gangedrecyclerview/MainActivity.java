...
public class MainActivity extends AppCompatActivity implements CheckListener {
    ...
    private void setChecked(int position, boolean isLeft) {
        ...
        if (isLeft) {
            mSortAdapter.setCheckedPosition(position);//选中左边列表
            //此处的位置需要根据每个分类的集合来进行计算
            int count = 0;
            for (int i = 0; i < position; i++) {
                count += mSortBean.getCategoryOneArray().get(i).getCategoryTwoArray().size();//右侧item的个数
            }
            count += position;//右侧item的个数,加上头
